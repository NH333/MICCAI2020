...
double Test(const int gid, int index)
{
    ...
    char path[100];
    sprintf(path, "/home/chenanqi/graph_comb_opt/data/mvc/simulate/resultSimulate/%d.txt",index);
...
int main(){
    ...
    int len_arr = 38;
    Init(len_arr,arr);
    std::vector<std::vector<std::vector<int>>> simulate_datas;
    std::vector<int> line;
    std::vector<std::vector<int>> data;
    std::ifstream retest_filename("/home/chenanqi/graph_comb_opt/data/mvc/simulate/file_path.txt");
...
//    std::string simluate_data_path = "/home/chenanqi/graph_comb_opt/data/mvc/simulate/1.csv";
//    std::ifstream input(simluate_data_path);
//    if(input){
//        int value;
//		int flag = 0;
//		while (input >> value)
...
//    }
    ...
    char* model_file = "/home/chenanqi/graph_comb_opt/data/batch128/model64/11100_iters.model";
    int n_valid = 119;
    int num_edges = 0;
    bool isTest = 1;
    int num_nodes = 148;
    ...
    //准备测试集数据
    for(int i=0;i<10;++i){
        num_edges = simulate_datas[valid_index[i]].size();
        for(int j=0;j<num_edges;++j){
            edges_from.push_back(simulate_datas[valid_index[i]][j][0]-1);
            edges_to.push_back(simulate_datas[valid_index[i]][j][1]-1);
        }
